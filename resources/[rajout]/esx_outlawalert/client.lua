...
AddEventHandler('outlawNotify', function(alert)
		if PlayerData.job ~= nil and PlayerData.job.name == 'police' or PlayerData.job.name == 'gouvernement' then
            Notify(alert)
        end
end)
...
Citizen.CreateThread( function()
    while true do
        ...
        if IsPedTryingToEnterALockedVehicle(GetPlayerPed(-1)) or IsPedJacking(GetPlayerPed(-1)) then
			TriggerServerEvent('eden_garage:debug', "[SERVEUR] Un Vole de Vehicule vien d'avoir lieux")
			Wait(3000)
			...
			if PlayerData.job ~= nil and PlayerData.job.name == 'police' or PlayerData.job.name == 'gouvernement' and showcopsmisbehave == false then
			elseif PlayerData.job ~= nil and PlayerData.job.name == 'police' or PlayerData.job.name == 'gouvernement' and showcopsmisbehave then
