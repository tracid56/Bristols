    ...
    while true do 
        ...
        if Vehicle.Vehicle ~= nil then
                ...
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) then
                   DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Appuyez sur ~b~E~w~ pour poussez le véhicule', 0.6) 
                end
     

            if IsControlPressed(0, Keys["E"]) and IsVehicleSeatFree(Vehicle.Vehicle, -1) and not IsEntityAttachedToEntity(ped, Vehicle.Vehicle) and GetLastInputMethod(2) then
