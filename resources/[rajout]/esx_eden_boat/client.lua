...
function ListVehiclesMenu(garage, KindOfVehicle)
	local elements = {}
	local vehicleName = ""
	ESX.TriggerServerCallback('eden_garage_boat:getVehicles', function(vehicles)
				...
				function(data2, menu2)
					if data2.current.value == "get_vehicle_out" then
                        if (data.current.value.fourrieremecano) then
                            TriggerEvent('esx:showNotification', 'Votre véhicule est dans la fourriere dépanneur')
					...
					elseif data2.current.value == "rename_vehicle" then
						...
						if (GetOnscreenKeyboardResult()) then
							...
							TriggerServerEvent('eden_garage_boat:renamevehicle', data.current.value.plate, name)
						end
					end
				end,
				function(data2, menu2)
...
function ListVehiclesFourriereMenu(garage)
	...
	ESX.TriggerServerCallback('eden_garage_boat:getVehiclesMecano', function(vehicles)
		...
		function(data, menu)
			...
			TriggerServerEvent('eden_garage_boat:ChangeStateFromFourriereMecano', data.current.value.vehicle, false)
		end,
		function(data, menu)
			menu.close()
		end
...
function StockVehicleMenu(KindOfVehicle)
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		local MinimumHealthPercent = 80

		if healthPercent < MinimumHealthPercent then
			TriggerEvent('esx:showNotification', 'Vous devez d\'abord faire réparer votre véhicule !')
		else
			if GetPedInVehicleSeat(vehicle, -1) == playerPed then
				...
				if GotTrailer then
					...
					ESX.TriggerServerCallback('eden_garage_boat:stockv',function(valid)
						if(valid) then
							...
							TriggerServerEvent('eden_garage_boat:modifystate', trailerProps.plate, true)
					...
					ESX.TriggerServerCallback('eden_garage_boat:stockv',function(valid)
						if(valid) then
							...
							TriggerServerEvent('eden_garage_boat:modifystate', vehicleProps.plate, true)
...
function StockVehicleFourriereMenu()
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		if GetPedInVehicleSeat(vehicle, -1) == playerPed then
			...
			if GotTrailer then
				local trailerProps  = ESX.Game.GetVehicleProperties(TrailerHandle)
				ESX.TriggerServerCallback('eden_garage_boat:stockvmecano',function(valid)
					if(valid) then
						DeleteVehicle(TrailerHandle)
						TriggerServerEvent('eden_garage_boat:ChangeStateFromFourriereMecano', trailerProps, true)
				...
				ESX.TriggerServerCallback('eden_garage_boat:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerServerEvent('eden_garage_boat:ChangeStateFromFourriereMecano', vehicleProps, true)
...
function SpawnVehicle(vehicle, garage, KindOfVehicle)
	...
	TriggerServerEvent('eden_garage_boat:modifystate', vehicle.plate, false)
end
...
function SpawnVehicleMecano(vehicle, garage)
	...
	TriggerServerEvent('eden_garage_boat:ChangeStateFromFourriereMecano', vehicle, false)
end
--Fin fonction pour spawn vehicule fourriere mecano

function ReturnVehicleMenu(garage, KindOfVehicle)

	ESX.TriggerServerCallback('eden_garage_boat:getOutVehicles', function(vehicles)
		local elements = {}
		if not table.empty(vehicles) then
			for _,v in pairs(vehicles) do
				v.vehicle = json.decode(v.vehicle)
		...
		function(data, menu)
			...
			elseif data.current.value ~= nil then
				...
				if not iscaronearth then
					ESX.TriggerServerCallback('eden_garage_boat:buy', function(hasEnoughMoney)
...
AddEventHandler('ft_libs:OnClientReady', function()
	for k,v in pairs (Config.Garages) do
		this_Garage = v
		exports.ft_libs:AddArea("esx_eden_garage_boat_area_"..k.."_garage", {
			marker = {
				weight = v.Marker.w,
				height = v.Marker.h,
				red = v.Marker.r,
		...
		exports.ft_libs:AddArea("esx_eden_garage_boat_area_"..k.."_spawnpoint", {
			marker = {
		...
		exports.ft_libs:AddArea("esx_eden_garage_boat_area_"..k.."_deletepoint", {
			marker = {
...
RegisterNetEvent('esx_eden_garage_boat:ListVehiclesMenu')
AddEventHandler('esx_eden_garage_boat:ListVehiclesMenu', function(garage, society)
	ListVehiclesMenu(garage, society)
end)

RegisterNetEvent('esx_eden_garage_boat:OpenMenuGarage')
AddEventHandler('esx_eden_garage_boat:OpenMenuGarage', function(garage, society)
	OpenMenuGarage(garage, society)
end)

RegisterNetEvent('esx_eden_garage_boat:StockVehicleMenu')
AddEventHandler('esx_eden_garage_boat:StockVehicleMenu', function(society)
