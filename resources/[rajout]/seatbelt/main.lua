  ...
  Citizen.Wait(0)
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
      wasInCar = true
             ...
             TriggerEvent('esx:showNotification', "Pensez a votre ~g~Ceinture")
                SendNUIMessage({
            	   displayWindow = 'true'
            	   })
                isUiOpen = true 			
      ...
      if IsControlJustReleased(0, 311) and GetLastInputMethod(2) then
        beltOn = not beltOn 
        if beltOn then 
        TriggerEvent('esx:showNotification', "Ceinture ~g~Attachée")
      ...
      TriggerEvent('esx:showNotification', "Ceinture ~r~Déttachée")
