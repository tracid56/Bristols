...
ESX.RegisterServerCallback('eden_garage_air:getVehicles', function(source, cb, KindOfVehicle)
...
--Recupere les véhicules
ESX.RegisterServerCallback('eden_garage_air:getVehiclesMecano', function(source, cb)
	local _source = source
	local vehicules = {}
...
--Stock les véhicules
ESX.RegisterServerCallback('eden_garage_air:stockv',function(source,cb, vehicleProps, KindOfVehicle)
	local identifier = ""
	local _source = source
	if KindOfVehicle ~= "personal" then
		identifier = KindOfVehicle
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles_boat where plate=@plate and owner=@identifier",{['@plate'] = vehplate, ['@identifier'] = identifier}, function(result)  
		if result[1] ~= nil then
			...
			if originalvehprops.model == vehiclemodel then
				...
				print("[esx_eden_garage_air] player "..identifier..' tried to spawn a vehicle with hash:'..vehiclemodel..". his original vehicle: "..originalvehprops.model)
				cb(false)
			end
		else
			cb(false)
...
ESX.RegisterServerCallback('eden_garage_air:stockvmecano',function(source,cb, vehicleProps)
	local _source = source
...
RegisterServerEvent('eden_garage_air:modifystate')
AddEventHandler('eden_garage_air:modifystate', function(plate, state)
...
RegisterServerEvent('eden_garage_air:ChangeStateFromFourriereMecano')
AddEventHandler('eden_garage_air:ChangeStateFromFourriereMecano', function(vehicleProps, fourrieremecano)
	local _source = source
...
RegisterServerEvent('eden_garage_air:renamevehicle')
AddEventHandler('eden_garage_air:renamevehicle', function(vehicleplate, name)
...
ESX.RegisterServerCallback('eden_garage_air:getOutVehicles',function(source, cb, KindOfVehicle)	
	local _source = source
	local vehicules = {}
	local identifier = ""
	if KindOfVehicle ~= "personal" then
...
ESX.RegisterServerCallback('eden_garage_air:buy', function(source, cb)
