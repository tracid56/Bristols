RegisterNetEvent('esx_fauteuil:add')
AddEventHandler('esx_fauteuil:add', function()
	LoadModel('prop_wheelchair_01')

	local wheelchair = CreateObject(GetHashKey('prop_wheelchair_01'), GetEntityCoords(PlayerPedId()), true)
end, false)

RegisterNetEvent('esx_fauteuil:remove')
AddEventHandler('esx_fauteuil:remove', function()
	...
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			if GetDistanceBetweenCoords(pedCoords, sitCoords, true) <= 1.0 then
				DrawText3Ds(sitCoords, "Appuyez sur ~b~E~w~ pour vous asseoir", 0.4)
			...
			if GetDistanceBetweenCoords(pedCoords, pickupCoords, true) <= 1.0 then
				DrawText3Ds(pickupCoords, "Appuyez sur ~b~E~w~ pousser le fauteuil", 0.4)
...
Sit = function(wheelchairObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'missfinale_c2leadinoutfin_c_int', '_leadin_loop2_lester', 3) then
			ShowNotification("Quelqu'un utilise déjà le fauteuil roulant !")
			return
		end
	end
...
PickUp = function(wheelchairObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'anim@heists@box_carry@', 'idle', 3) then
			ShowNotification("Quelqu'un pousse déjà le fauteuil roulant!")
			return
		end
	end
...
DrawText3Ds = function(coords, text, scale)
	...
	-- DrawRect(_x, _y + 0.0150, 0.030 + factor, 0.025, 255, 255, 255, 255)
end
