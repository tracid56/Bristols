...
function RemoveOwnedVehicle (plate)
  MySQL.Async.fetchAll(
    'SELECT * FROM owned_vehicles_boat',
    {},
    function (result)
      for i=1, #result, 1 do
        ...
        if vehicleProps.plate == plate then
          MySQL.Async.execute(
            'DELETE FROM owned_vehicles_boat WHERE id = @id',
            { ['@id'] = result[i].id }
          )
        end
      end
...
AddEventHandler('onMySQLReady', function ()
  Categories       = MySQL.Sync.fetchAll('SELECT * FROM vehicle_categories_boat')
  local vehicles   = MySQL.Sync.fetchAll('SELECT * FROM vehicles_boat')
...
AddEventHandler('esx_boatshop:setVehicleOwned', function (vehicleProps)
  ...
  MySQL.Async.execute(
    'INSERT INTO owned_vehicles_boat (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)',
    {
...
ESX.RegisterServerCallback('esx_boatshop:getPersonnalVehicles', function (source, cb)
  ...
  MySQL.Async.fetchAll(
    'SELECT * FROM owned_vehicles_boat WHERE owner = @owner',
    { ['@owner'] = xPlayer.identifier },
    function (result)
      local vehicles = {}
