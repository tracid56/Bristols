...
Citizen.CreateThread(function()
  while true do
    ...
    Wait(0)
    if IsControlPressed(0, Keys["L"]) and (GetGameTimer() - GUI.Time) > 1000 and GetLastInputMethod(2) then
		if count == 0 then
			openmenuvehicle()
			count = count +1
		else
              ...
              if not max then
                  ...
                  ESX.ShowNotification('Poid du coffre: ~g~'.. Kgweight .. ' Kg / '..MaxVh..' Kg')
				        -- Citizen.Wait(500)
									TriggerServerEvent("esx_truck_inventory:getInventory", GetVehicleNumberPlateText(vehFront))
									nocheat = false
								elseif not nocheat then
									ESX.ShowNotification('~r~C\'est pas bien de cheat !')
							  end
				      elseif max then
