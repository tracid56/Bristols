...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		local elements = {
			{label = 'Véhicule de Travail',  value = 'benson'},
			{label = 'Bateau',  value = 'tug'},
		}
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				if data.current.value == 'benson' then
				    ESX.Game.SpawnVehicle('benson', Config.Zones.VehicleSpawnPoint.Pos, 56.326, function(vehicle)
					    local playerPed = GetPlayerPed(-1)
					    TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
					end)
				end

				if data.current.value == 'tug' then
					ESX.Game.SpawnVehicle('tug', Config.Zones.BateauSpawnPoint.Pos, 56.326, function(vehicle)
					    local playerPed = GetPlayerPed(-1)
					    TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
					end)
				end
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_pecheur', 'Poisson', amount)
								end)
							end
						end
					end,
...
AddEventHandler('esx_pecheurjob:hasEnteredMarker', function(zone)
	if zone == 'RaisinFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementVin' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementJus' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur'  then
		CurrentAction     = 'jus_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'VigneronActions' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
		CurrentAction     = 'vigne_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
...
AddEventHandler('esx_pecheurjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'RaisinFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
		TriggerServerEvent('esx_pecheurjob:stopHarvest')
	end  
	if (zone == 'TraitementVin' or zone == 'TraitementJus') and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
		TriggerServerEvent('esx_pecheurjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
		TriggerServerEvent('esx_pecheurjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		...
		AddTextComponentString("Pêcheur")
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_pecheurjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_pecheurjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					TriggerServerEvent('esx_pecheurjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'jus_traitement' then
					TriggerServerEvent('esx_pecheurjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'vine_traitement' then
					TriggerServerEvent('esx_pecheurjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_pecheurjob:startSell', CurrentActionData.zone)
				end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' and (GetGameTimer() - GUI.Time) > 150 then
