$(document).ready(function() {
  ...
  $('#date').text(d.getDate() + '/' +((''+month).length<2 ? '0' : '') + month + '/' +((''+ans).length<2 ? '0' : '') + ans);
  ...
  function getUser(dob) {
    ...
    $.post('http://jsfour-criminalrecord/fetch', JSON.stringify({
      ...
      if ( cb != 'error') {
        ...
        $('.r-dob-year').html('<i>Jours</i>' + dob.substr(0,2));
        $('.r-dob-month').html('<i>Mois</i>' + dob.substr(3,2));
        $('.r-dob-day').html('<i>Année</i>' + dob.substr(8,2));
      ...
      if( !$(this).hasClass('user-search') ) {
        $.post('http://jsfour-criminalrecord/fetch', JSON.stringify({
          ...
          if ( cb != 'error' ) {
            ...
            $('.r-year').html('<i>Ans</i>' + parseInt(term[0]));
            $('.r-month').html('<i>Mois</i>' + parseInt(term[1]));
            $('.r-day').html('<i>Jours</i>' + parseInt(term[2]));
  ...
  $('#submit-search').click(function() {
    ...
    $('#search').hide();
    $('#search-result').show();
    $('#search-result p').text('Recherche...');
    ...
    if ( offense.length > 0 ) {
      $.post('http://jsfour-criminalrecord/fetch', JSON.stringify({
        ...
        if ( cb != 'error' ) {
          ...
          $('#search-result p').text('Aucun Résultat');
        }
      });
    } else {
