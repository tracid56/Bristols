...
AddEventHandler('esx_gym:useBandage', function()
	...
	ESX.ShowNotification("Vous avez utilise un ~g~bandage")
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour prendre une ~b~bicyclette')
				
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("Vous avez déjà une bicyclette ~r~")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ exercer votre ~g~bras')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Préparer l'~g~exercice~w~...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							ESX.ShowNotification("Vous devez vous reposer environ ~r~60 secondes ~w~avant de faire un autre exercice")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Vous avez besoin d'un abonnement pour faire un ~r~exercice")
						end
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour faire quelques ~g~ tractions')
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then
							...
							Citizen.Wait(30000)
							ClearPedTasksImmediately(playerPed)
							ESX.ShowNotification("Préparer l'Vous devez vous reposer environ ~r~60 secondes ~w~avant de faire un autre exercice")
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour faire quelques ~g~ pompes')
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Nous sommes ouverts ~g~ 24/24 Heures 7/7 Jours~w~. Bienvenue!")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
			...
			align = 'top-left',
            elements = {
				{label = 'Protéine ($6)', value = 'protein_shake'},
				{label = 'Eau ($1)', value = 'water'},
				{label = 'Déjeuner Sportif ($2)', value = 'sportlunch'},
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_ship_menu',
        {
			...
			align = 'top-left',
            elements = {
				{label = 'Adhésion ($800)', value = 'membership'},
            }
        },
        function(data, menu)
