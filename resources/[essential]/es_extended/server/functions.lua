...
ESX.SavePlayer = function(xPlayer, cb)
	...
	table.insert(asyncTasks, function(cb)
		MySQL.Async.execute('UPDATE users SET `job` = @job, `job_grade` = @job_grade, `faction` = @faction, `faction_grade` = @faction_grade, `loadout` = @loadout, `position` = @position WHERE identifier = @identifier', {
...
ESX.DoesFactionExist = function(faction, grade)
	...
	if faction and grade then
		if ESX.Faction[faction] and ESX.Faction[faction].grades[grade] then
			return true
		end
	end
