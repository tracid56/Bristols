...
MySQL.ready(function()
	...
	local resultfaction = MySQL.Sync.fetchAll('SELECT * FROM faction', {})

	for i=1, #resultfaction do
		ESX.Faction[resultfaction[i].name] = resultfaction[i]
		ESX.Faction[resultfaction[i].name].grades = {}
	end

	local resultfaction2 = MySQL.Sync.fetchAll('SELECT * FROM faction_grades', {})

	for i=1, #resultfaction2 do
		if ESX.Faction[resultfaction2[i].faction_name] then
			ESX.Faction[resultfaction2[i].faction_name].grades[tostring(resultfaction2[i].grade)] = resultfaction2[i]
		else
			print(('es_extended: invalid faction "%s" from table job_grades ignored!'):format(resultfaction2[i].job_name))
		end
	end

	for k,v in pairs(ESX.Faction) do
		if next(v.grades) == nil then
			ESX.Faction[v.name] = nil
