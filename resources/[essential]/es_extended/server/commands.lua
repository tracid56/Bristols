	...
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			if ESX.DoesJobExist(args[2], args[3]) then
				...
				TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Ce métier n\'existe pas' } })
			end

		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Le joueur n\'est pas en ligne' } })
		end
	else
		TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Utilisation non valide' } })
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Permissions insuffisantes' } })
...
TriggerEvent('es:addGroupCommand', 'setfaction', 'superadmin', function(source, args, user)
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			if ESX.DoesFactionExist(args[2], args[3]) then
				xPlayer.setFaction(args[2], args[3])
			else
				TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Cette faction n\'existe pas' } })
			end
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Permissions insuffisantes' } })
end, {help = _U('setfaction'), params = {{name = "id", help = _U('id_param')}, {name = "faction", help = _U('setfaction_param2')}, {name = "grade_id", help = _U('setfaction_param3')}}})
