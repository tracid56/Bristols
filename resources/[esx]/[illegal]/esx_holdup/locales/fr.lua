Locales['fr'] = {

	['robbery_cancelled'] = 'Le braquage vas être annulé, vous ne gagnerez rien !',
	['robbery_successful'] = 'Braquage réussi, vous avez gagné ~g~$',
	['store_robbery'] = 'Braquage magasin',
	['press_to_rob'] = 'Appuyez sur ~INPUT_CONTEXT~ pour lancer un braquage ~b~',
	['robbery_of'] = 'Braquage de magasin ~r~',
	['seconds_remaining'] = '~w~ secondes restantes',
	['robbery_cancelled_at'] = '~r~Braquage annulé ~b~',
	['robbery_has_cancelled'] = 'Le braquage à été annulé ~b~',
	['already_robbed'] = 'Ce magasin a déjà été braqué. Veuillez attendre ',
	['seconds'] = ' secondes',
	['rob_in_prog'] = '~r~Braquage en cours ~b~',
	['started_to_rob'] = 'vous avez commencé à braquer ',
	['do_not_move'] = ', ne vous éloignez pas !',
	['alarm_triggered'] = 'l\'alarme à été déclenché',
	['hold_pos'] = 'Tenez la position pendant 5min et l\'argent est à vous !',
	['robbery_complete'] = '~r~Braquage terminé et vous avez gagnez ~g~',
	['robbery_complete_at'] = '~r~Braquage terminé ~b~',
	['min_two_police'] = 'Il faut minimum ',
	['robbery_already'] = '~r~Un braquage est déjà en cours',

}
