...
AddEventHandler('esx:playerLoaded', function(xPlayer)
  	PlayerData = xPlayer
	ESX.TriggerServerCallback('esx_lscustommeca:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_lscustommeca:installMod')
AddEventHandler('esx_lscustommeca:installMod', function()
	--Citizen.Trace('installMod')
	local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_lscustommeca:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_lscustommeca:cancelInstallMod')
AddEventHandler('esx_lscustommeca:cancelInstallMod', function()
...
function OpenLSMenu(elems, menuname, menutitle, parent)
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu) -- on cancel
			menu.close()
			TriggerEvent('esx_lscustommeca:cancelInstallMod')
				...
				elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
					...
					for j = 0, modCount, 1 do
						...
						if j == currentMods[k] then
							...
							price = math.floor(vehiclePrice * v.price[j+1] / 150)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local zone 		  = nil
			local lastZone    = nil
			if (PlayerData.job ~= nil and PlayerData.job.name == 'pdm' ) or Config.IsPdmJobOnly == false then
				for k,v in pairs(Config.Zones) do
