...
function OpenVehicleSpawnerMenu()
	...
	local elements = {}
	table.insert(elements, { label = 'Camion', value = 'stockade' })
	...
	function(data, menu)
		...
		ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 182.0, function(vehicle)
			SetVehicleMaxMods(vehicle)
			SetVehicleNumberPlateText(vehicle, "BRINK" .. platenum)
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_brinks', 'Transport de fond', amount)
								end)
							end
						end
					end,
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
    ...
    TriggerServerEvent("player:serviceOn", "brinks")
  end
	blips()
end)
...
AddEventHandler('esx_brinks:hasEnteredMarker', function(zone)
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'brinks'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'VigneronActions' and PlayerData.job ~= nil and PlayerData.job.name == 'brinks' then
		CurrentAction     = 'vigne_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'brinks' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'brinks' then
...
function blips()
	if publicBlip == false then
		...
		AddTextComponentString("Brinks")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'brinks' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'brinks' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					TriggerServerEvent('esx_brinks:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'jus_traitement' then
					TriggerServerEvent('esx_brinks:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'vine_traitement' then
					TriggerServerEvent('esx_brinks:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_brinks:startSell', CurrentActionData.zone)
				end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'brinks' and (GetGameTimer() - GUI.Time) > 150 then
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlJustReleased(1, Keys["DELETE"]) and PlayerData.job ~= nil and PlayerData.job.name == 'brinks' and GetLastInputMethod(2) then
