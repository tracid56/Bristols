	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 56.0, function(vehicle)
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_tabac', 'Cigarette', amount)
								end)
							end
						end
					end,
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	if PlayerData.job.name == 'tabac' then
...
AddEventHandler('esx_tabac:hasEnteredMarker', function(zone)
	if zone == 'RaisinFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'tabac'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementVin' and PlayerData.job ~= nil and PlayerData.job.name == 'tabac'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementJus' and PlayerData.job ~= nil and PlayerData.job.name == 'tabac'  then
...
AddEventHandler('esx_tabac:hasExitedMarker', function(zone)
	...
	if (zone == 'TraitementVin' or zone == 'TraitementJus') and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
