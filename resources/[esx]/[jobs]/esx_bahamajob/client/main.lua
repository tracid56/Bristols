...
function OpenBahamaActionsMenu() 
    ...
    {label = 'Prendre un lanceur Pyro', value = 'givepyro'},
    {label = 'Rendre le lanceur Pyro', value = 'removepyro'}
	}
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'givepyro' then
        TriggerServerEvent('esx_bahamajob:giveWeapon', "WEAPON_FIREWORK",  1000)
      end
      
      if data.current.value == 'removepyro' then
        TriggerServerEvent('esx_bahamajob:removeWeapon', "WEAPON_FIREWORK")
      end
