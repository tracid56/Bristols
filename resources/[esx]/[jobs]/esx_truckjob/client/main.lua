	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'truck', function(data, menu)
					menu.close()
				end)
			end
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'truck', vehicleProps)
		...
		local elements = {
			...
			{label = 'Remorque (Faire spawn le camion avant)',  value = 'trailers3'},
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_truck', 'Chauffeur', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_truck:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_truck:getStockItem', itemName, count)
						end
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_truck:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_truck:hasEnteredMarker', function(zone)
	if zone == 'RaisinFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'truck'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementVin' and PlayerData.job ~= nil and PlayerData.job.name == 'truck'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementJus' and PlayerData.job ~= nil and PlayerData.job.name == 'truck'  then
		CurrentAction     = 'jus_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'truck'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'VigneronActions' and PlayerData.job ~= nil and PlayerData.job.name == 'truck' then
		CurrentAction     = 'vigne_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'truck' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'truck' then
...
AddEventHandler('esx_truck:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'RaisinFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'truck' then
		TriggerServerEvent('esx_truck:stopHarvest')
	end  
	if (zone == 'TraitementVin' or zone == 'TraitementJus') and PlayerData.job ~= nil and PlayerData.job.name == 'truck' then
		TriggerServerEvent('esx_truck:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'truck' then
		TriggerServerEvent('esx_truck:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		...
		AddTextComponentString("Chauffeur Routier")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'truck' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'truck' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_truck:hasEnteredMarker', currentZone)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'truck' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					TriggerServerEvent('esx_truck:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'jus_traitement' then
					TriggerServerEvent('esx_truck:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'vine_traitement' then
					TriggerServerEvent('esx_truck:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_truck:startSell', CurrentActionData.zone)
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'truck', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'truck' and (GetGameTimer() - GUI.Time) > 150 then
