...
RegisterServerEvent('esx_vigneronjob:annonce')
AddEventHandler('esx_vigneronjob:annonce', function(result)
	local _source  = source
	...
	for i=1, #xPlayers, 1 do
 		...
 		TriggerClientEvent('esx_vigneronjob:annonce', xPlayers[i],text)
	end
	...
	for i=1, #xPlayers, 1 do
 		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
 		TriggerClientEvent('esx_vigneronjob:annoncestop', xPlayers[i])
	end
