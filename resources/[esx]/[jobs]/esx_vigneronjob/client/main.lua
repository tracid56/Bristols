...
function Message()
    ...
    while messagenotfinish do
		    ...
		    if (GetOnscreenKeyboardResult()) then
		       ...
		       TriggerServerEvent('esx_vigneronjob:annonce',result)
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'citizen_wear' then
				TriggerServerEvent("player:serviceOff", "vigne")
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 90.0, function(vehicle)
						...
						SetVehicleNumberPlateText(vehicle, "VGN" .. platenum)
...
RegisterNetEvent('esx_vigneronjob:annonce')
AddEventHandler('esx_vigneronjob:annonce', function(text)
 		texteafiche = text
 		affichenews = true
...
RegisterNetEvent('esx_vigneronjob:annoncestop')
AddEventHandler('esx_vigneronjob:annoncestop', function()
 		affichenews = false
