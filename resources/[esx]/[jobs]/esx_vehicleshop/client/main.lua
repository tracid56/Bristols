    ...
    while messagenotfinish do
      ...
      DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "", "", "", "", "", 100)
        ...
        if (GetOnscreenKeyboardResult()) then
           ...
           TriggerServerEvent('esx_vehicleshop:annonce',result)
					...
					function(data, menu)
						...
						if amount == nil then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_pdm', 'pdm', amount)
							end
						end
					end,
				function(data, menu)
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'pdm' then
                if CurrentAction == 'OpenResellerMenu' then
                ...
                if CurrentAction == 'delete_vehicle' then
                    ...
                    if hash == GetHashKey('flatbed') or hash == GetHashKey('sandking') or hash == GetHashKey('slamvan3') or hash == GetHashKey('forklift') then
                        if Config.MaxInService ~= -1 then
                            TriggerServerEvent('esx_service:disableService', 'pdm')
                        end
        ...
        if IsControlJustReleased(0, Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'pdm' then
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'reseller',
    {
      ...
      elements = {
        ...
        {label = 'Véhicule de fonction', value = 'vehicle_list'},
        {label = 'Passer une annonce',                 value = 'announce'},
    ...
    function (data, menu)
      if data.current.value == 'vehicle_list' then
				local elements = {
					...
					--{label = 'kalahari pour les client', value = 'kalahari'}
				}

				if Config.EnablePlayerManagement and PlayerData.job ~= nil and 
					(PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'chief' or PlayerData.job.grade_name == 'experimente') then
						...
						for i=1, #elements, 1 do							
							if Config.MaxInService == -1 then
								...
								ESX.Game.SpawnVehicle(data.current.value, coords, 90.0, function(vehicle)
									SetVehicleNumberPlateText(vehicle, "Bens" .. platenum)
									TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
									local plate = GetVehicleNumberPlateText(vehicle)
          ...
          function (data2, menu)
            ...
            if amount == nil then
              ...
              if closestPlayer == -1 or closestDistance > 3.0 then
                ...
                TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_pdm', _U('car_dealer'), tonumber(data2.value))
              end
            end
          end,
          function (data2, menu)
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'citizen_wear' then
        ...
        TriggerServerEvent("player:serviceOff", "pdm")
      ...
      if data.current.value == 'agent_wear' then
        ...
        TriggerServerEvent("player:serviceOn", "pdm")
        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
...
AddEventHandler('esx:playerLoaded', function (xPlayer)
  ...
  if Config.EnablePlayerManagement then
    if PlayerData.job.name == 'pdm' then
...
AddEventHandler('esx_vehicleshop:hasEnteredMarker', function (zone)
  if zone == 'ShopEntering' then
    if Config.EnablePlayerManagement then
      if PlayerData.job ~= nil and PlayerData.job.name == 'pdm' then
        CurrentAction     = 'reseller_menu'
        CurrentActionMsg  = _U('shop_menu')
        CurrentActionData = {}
      end
  ...
  if zone == 'BossActions' and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'pdm' and PlayerData.job.grade_name == 'boss' then
    CurrentAction     = 'boss_actions_menu'
    CurrentActionMsg  = _U('shop_menu')
    CurrentActionData = {}
  end
        ...
        while true do
            Citizen.Wait(1)    
                ...
                if (affichenews == true) then
                    ...
                    DrawAdvancedTextCNN(0.588, 0.14, 0.005, 0.0028, 0.8, "~r~ PREMIUM MOTORSPORT ~d~", 255, 255, 255, 255, 1, 0)
...
RegisterNetEvent('esx_vehicleshop:annonce')
AddEventHandler('esx_vehicleshop:annonce', function(text)
    texteafiche = text
    affichenews = true
...
RegisterNetEvent('esx_vehicleshop:annoncestop')
AddEventHandler('esx_vehicleshop:annoncestop', function()
    affichenews = false
    
  end)

  local Despawnpoint = {
    { ['x'] = -45.692226409912, ['y'] = -1082.8703613281, ['z'] = 26.841680526733 }
  }
