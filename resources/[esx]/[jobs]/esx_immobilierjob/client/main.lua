    ...
    while messagenotfinish do
      ...
      DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "", "", "", "", "", 44)
        ...
        if (GetOnscreenKeyboardResult()) then
            ...
            TriggerServerEvent('esx_immobilierjob:immobillier',result)
...
function OpenRealestateAgentMenu()
  ...
  if PlayerData.job ~= nil and PlayerData.job.name == 'immobilier' and PlayerData.job.grade_name == 'boss' then
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'boss_actions' then
        TriggerEvent('esx_society:openBossMenu', 'immobilier', function(data, menu)
          menu.close()
        end)
      end
            ...
            function(data2, menu)
              ...
              if amount == nil then
                ...
                if closestPlayer == -1 or closestDistance > 3.0 then
                  ...
                  TriggerServerEvent('esx_immobilierjob:sell', GetPlayerServerId(closestPlayer), data.data.name, amount)
                end
            ...
            function(data2, menu)
              ...
              if amount == nil then
                ...
                if closestPlayer == -1 or closestDistance > 3.0 then
                  ...
                  TriggerServerEvent('esx_immobilierjob:rent', GetPlayerServerId(closestPlayer), data.data.name, amount)
                end
...
function OpenMobileImmobilierActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_immobilier_actions',
		{
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
							  ...
							  local playerPed        = GetPlayerPed(-1)
								...
								Citizen.CreateThread(function()
								    ...
								    TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_immobilier', 'Agent Immobilier', amount)
								end)
							end
...
function OpenCustomersMenu()
  ...
  ESX.TriggerServerCallback('esx_immobilierjob:getCustomers', function(customers)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.value == 'revoke' then
          TriggerServerEvent('esx_immobilierjob:revoke', data.data.propertyName, data.data.propertyOwner)
          OpenCustomersMenu()
        end
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
  ...
  if PlayerData.job.name == 'immobilier' then
    TriggerServerEvent("player:serviceOn", "immobilier")
...
AddEventHandler('esx_immobilierjob:hasEnteredMarker', function(zone)
  ...
  if zone == 'OfficeActions' and PlayerData.job ~= nil and PlayerData.job.name == 'immobilier' then
...
AddEventHandler('esx_immobilierjob:hasExitedMarker', function(zone)
  ESX.UI.Menu.CloseAll()
  CurrentAction = nil
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
      ...
      TriggerEvent('esx_immobilierjob:hasEnteredMarker', currentZone)
    end

    if not isInMarker and HasAlreadyEnteredMarker then
      HasAlreadyEnteredMarker = false
      TriggerEvent('esx_immobilierjob:hasExitedMarker', LastZone)
    end

  end
end)

if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'immobilier' and (GetGameTimer() - GUI.Time) > 150 then
  OpenMobileImmobilierActionsMenu()
