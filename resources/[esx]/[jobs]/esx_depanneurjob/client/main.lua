...
function mecano()
    ...
    while messagenotfinish do
      ...
      AddTextEntry('FMMC_MPM_NA', "Message de l'annonce")
            ...
            TriggerServerEvent('esx_mecanojob:mecano',result)
			...
			function(data, menu)
				if data.current.value == 'vehicle_list' then
					...
					if Config.EnableSocietyOwnedVehicles then
							...
							ESX.UI.Menu.Open(
								...
								function(data, menu)
									...
									ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
										...
										SetVehicleNumberPlateText(vehicle, "DEPA" .. platenum)
						...
						if Config.EnablePlayerManagement and ESX.PlayerData.job ~= nil and
							(ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'chef' or ESX.PlayerData.job.grade_name == 'experimente') then
							...
							function(data, menu)
								for i=1, #elements, 1 do
									if Config.MaxInService == -1 then
										ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 234.0, function(vehicle)
											local playerPed = GetPlayerPed(-1)
											TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
										end)
										break
...
AddEventHandler('esx_mecanojob:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mecano' and not IsPedInAnyVehicle(playerPed, false) then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, Keys['E']) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mecano' then
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mecano' then
			OpenMobileMechanicActionsMenu()
		end

		if IsControlJustReleased(0, Keys['DELETE']) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mecano' then
			if NPCOnJob then
