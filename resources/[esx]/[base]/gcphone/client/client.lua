...
function showFixePhoneHelper (coords)
  for number, data in pairs(FixePhone) do
    ...
    if dist <= 2.0 then
      ...
      AddTextComponentString("" .. data.name .. ' ~r~' .. number .. '~n~~INPUT_PICKUP~~w~ Utiliser')
    ...
    for i, _ in pairs(PhoneInCall) do 
        ...
        if (dist <= 150) then
          DrawMarker(1, PhoneInCall[i].coords.x, PhoneInCall[i].coords.y, PhoneInCall[i].coords.z,
              0,0,0, 0,0,0, 2.0,2.0,2.0, 0,255,0,255, 0,0,0,0,0,0,0)
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if IsControlJustPressed(1, KeyOpenClose) and GetLastInputMethod(2) then -- F2
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 then
    ...
    if ShowNumberNotification == true then
      text = 'Nouveau message du ~y~'.. message.transmitter
    ...
    SetNotificationTextEntry("STRING")
    AddTextComponentString(text)
    SetNotificationMessage("CHAR_CHAT_CALL", "CHAR_CHAT_CALL", true, 1, "Téléphone", "~r~Bristols");
