...
function OpenDMVSchoolMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'dmvschool_actions',
	{
		...
		if data.current.value == 'theory_test' then
			ESX.TriggerServerCallback('esx_dmvschool:paytheory', function(hasEnoughMoney)
		...
		if data.current.value == 'drive_test' then
		    if data.current.type == 'drive' then
			    ESX.TriggerServerCallback('esx_dmvschool:paydrive', function(hasEnoughMoney)
                    if hasEnoughMoney then
					    menu.close()
			...
			elseif data.current.type == 'drive_bike' then
			    ESX.TriggerServerCallback('esx_dmvschool:paydrivebike', function(hasEnoughMoney)
                    if hasEnoughMoney then
					    menu.close()
					    StartDriveTest(data.current.type)
                    else
			...
			elseif data.current.type == 'drive_truck' then
			    ESX.TriggerServerCallback('esx_dmvschool:paydrivetruck', function(hasEnoughMoney)
				    if hasEnoughMoney then
					    menu.close()
					    StartDriveTest(data.current.type)
				    else
